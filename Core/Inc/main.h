...
#define WHEEL_LENGTH 20 //pi * diameter of wheel = (Pi*7)
#define PPR 330
...
#define INIT_DUTY_SP2_L 4000
#define INIT_DUTY_SP2_R 4000
#define DUTY_SP2_RANGE 700

#define DIR_FORWARD 1
#define DIR_BACKWARD 0

#define SERVO_LEFT_MAX 40
#define SERVO_CENTER 75
#define SERVO_RIGHT_MAX 110
...
#define __GET_DISTFROMTICK(tempdist, vertDist, targetTick) ({ \
	tempDist = ((targetTick + 10) / 1320 * WHEEL_LENGTH + DIST_C)/ (DIST_M); \
	vertDist += tempdist; \
})
...
#define __CALCULATE_RETURN_DIST(vertDist, horiDist, retDist) ({ \
	 retDist = sqrt(pow(vertDist,2) + pow(horiDist,2));\
})

#define __RETURN_HOME_ANGLE_LEFT(vertDist,horiDist, retAngle)({ \
		retAngle = 90 - atan((vertDist)/(horiDist)) * (180/M_PI); \
})

#define __RETURN_HOME_ANGLE_RIGHT(vertDist,horiDist, retAngle)({ \
		retAngle = 90 - atan((vertDist)/(horiDist)) * (180/M_PI); \
})
