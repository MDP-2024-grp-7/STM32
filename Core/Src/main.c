...
CmdConfig cfgs[19] = {
		...
		{800, 1200, 45, 0, DIR_FORWARD}, // FL--
		{1200, 800, 85, 0, DIR_FORWARD}, // FR--
		{800, 1200, 45, 0, DIR_BACKWARD}, // BL--
		{1200, 800, 85, 0, DIR_BACKWARD}, // BR--
//      LeftDuty, RightDuty, ServoTurnVal, TargetAngle
		{400, 1400, 45, 87, DIR_FORWARD}, // FL00
		{1800, 600, 105, -89, DIR_FORWARD}, // FR00
		{400, 2000, 45, -89, DIR_BACKWARD}, // BL00
		{2000, 400, 105, 87, DIR_BACKWARD}, // BR00
//problem child
		{0, 0, 110, 0, DIR_FORWARD}, // FL20
		{0, 0, 50, 0, DIR_FORWARD}, // FR20
		{700, 1800, 110, -89, DIR_BACKWARD}, // BL20
		{1800, 700, 175, 89, DIR_BACKWARD}, // BR20,

		{400, 3000, 110, 87.5, DIR_FORWARD}, // FL30
		{3000, 400, 175, -86.5, DIR_FORWARD}, // FR30
		{400, 3000, 110, -87.5, DIR_BACKWARD}, // BL30
		{3000, 400, 175, 88, DIR_BACKWARD}, // BR30
};
...
uint16_t obsTick_IR1 = 0, obsTick_IR2 = 0;

float obsDist_IR1 = 0, obsDist_US = 0, obsDist_IR2 = 0;
//float IR_data_raw_acc = 0, dataPoint = 0;
uint16_t dataPoint1 = 0, dataPoint2 = 0; uint32_t IR_data_raw_acc1 = 0, IR_data_raw_acc2 = 0;
float speedScale = 1;
...
void RobotMoveDistObstacle(float * targetDist, const uint8_t speedMode, int recordDistance);
...
{
	if (htim->Channel == HAL_TIM_ACTIVE_CHANNEL_1)  // if the interrupt source is channel2 (TRI: TIM4_CH2)
	{
		if (Is_First_Captured==0) // if the first value is not captured on UART
		{
		...
		else if (Is_First_Captured==1)   // if the first is already captured UART
		{
...
//	HAL_TIM_Encoder_Start(&htim3, TIM_CHANNEL_ALL);
  ...
  StartDefaultTasHandle = osThreadNew(defaultTask, NULL, &StartDefaultTas_attributes);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef * huart) {
	...
	UNUSED(huart);
	int val;
	/////////Messages come in 4 bytes eg. FW30. First 2 bytes determine command, last 2 are saved in cmd.val to be used in tasks//////////
	...
	else if (aRxBuffer[0] == 'W' && aRxBuffer[1] == 'X') __ADD_COMMAND(cQueue, 16, val); // For video demo
...
void StraightLineMove(const uint8_t speedMode) { //video
...
void RobotMoveDist(float * targetDist, const uint8_t dir, const uint8_t speedMode) {
	...
	__GET_TARGETTICK(*targetDist, targetDistTick); //turns target distance into ticks
...
void RobotMoveDistObstacle(float * targetDist, const uint8_t speedMode, int recordDistance) {
	angleNow = 0; gyroZ = 0;
	PIDConfigReset(&pidTSlow); //TO RECORD DIST//
	PIDConfigReset(&pidSlow); //TO RECORD DIST//
	PIDConfigReset(&pidFast); //TO RECORD DIST//
	obsDist_US = 1000;
	curDistTick = 0; //TO RECORD DIST//
	HAL_TIM_IC_Start_IT(&htim3, TIM_CHANNEL_1);
	last_curTask_tick = HAL_GetTick();
	__SET_ENCODER_LAST_TICK(&htim2,lastDistTick_L); //TO RECORD DIST//
	do {
	  HAL_GPIO_WritePin(Trig_GPIO_Port, Trig_Pin, GPIO_PIN_SET);  // pull the TRIG pin HIGH
	  __delay_us(&htim3, 10); // wait for 10us
	  HAL_GPIO_WritePin(Trig_GPIO_Port, Trig_Pin, GPIO_PIN_RESET);  // pull the TRIG pin low
	  __HAL_TIM_ENABLE_IT(&htim3, TIM_IT_CC1);
	  osDelay(10); // give timer interrupt chance to update obsDist_US value
	  if (abs(*targetDist - obsDist_US) < 0.1) {//task2
		  __GET_ENCODER_TICK_DELTA(&htim2, lastDistTick_L, dist_dL); //TO RECORD DIST//
		  curDistTick += dist_dL; //TO RECORD DIST//
		  tempDist = ((double)curDistTick/15435)*200; //convert ticks moved into dist //TO RECORD DIST//
		  if (recordDistance == 1) {
			  vertDist = tempDist; //TO RECORD DIST//
			  osDelay(10); //TO RECORD DIST//
		  }
		  //RobotMoveDist((float)&tempDist, DIR_BACKWARD, SPEED_MODE_1);
		  break;
	  }
	  __SET_MOTOR_DIRECTION(obsDist_US >= *targetDist);
...
void FASTPATH_LEFT_OBS1() {
	...
	__SET_MOTOR_DIRECTION(1);
	__SET_MOTOR_DUTY(&htim8, 2500, 1000);
	RobotTurn(&targetAngle);
	  osDelay(10);
	  targetDist = 20;
...
void FASTPATH_STRAIGHTLEFT_OBS1() {
	targetAngle = 60;
	__SET_SERVO_TURN(&htim1, 95);
	__SET_MOTOR_DIRECTION(0);
	__SET_MOTOR_DUTY(&htim8, 800, 2500);
	RobotTurn(&targetAngle);
}
void FASTPATH_RIGHT_OBS1() {
	targetAngle = -45;
	...
	__SET_SERVO_TURN(&htim1, 60);
	__SET_MOTOR_DIRECTION(1);
	__SET_MOTOR_DUTY(&htim8, 1000, 2500);
	RobotTurn(&targetAngle);
	  osDelay(10);
	  targetDist = 35;
	  RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_1);
...
void FASTPATH_STRAIGHTRIGHT_OBS1() {
	targetAngle = -60;
	__SET_SERVO_TURN(&htim1, 60);
	__SET_MOTOR_DIRECTION(0);
	__SET_MOTOR_DUTY(&htim8, 2500, 800);
	RobotTurn(&targetAngle);
}
...
void TURN_LEFT_X(float angle) { //To turn return to carpark
	targetAngle = angle - 0.05*angle;
	__SET_SERVO_TURN(&htim1, 60);
	__SET_MOTOR_DIRECTION(1);
	__SET_MOTOR_DUTY(&htim8, 800, 3000);
...
void TURN_RIGHT_X(float angle) { //To turn return to carpark
	targetAngle = -angle + 0.05*angle;
	__SET_SERVO_TURN(&htim1, 90);
	__SET_MOTOR_DIRECTION(1);
	__SET_MOTOR_DUTY(&htim8, 3000, 800);
...
void FASTESTPATH_TURN_RIGHT_180() {
	targetAngle = -178;	//From -176 to -178
	__SET_SERVO_TURN(&htim1, 110);
	__SET_MOTOR_DIRECTION(1);
	__SET_MOTOR_DUTY(&htim8, 3000, 800);
	RobotTurn(&targetAngle);
...
void RobotMoveUntilRIGHTIROvershootFT() { //Move until IR stops detecting. FT (First Turn) after robot detects arrow on second obstacle
	obsDist_IR2 = 0;
	angleNow = 0; gyroZ = 0;
	  last_curTask_tick = HAL_GetTick();
	  do {
		  __ADC_Read_Dist(&hadc3, dataPoint2, IR_data_raw_acc2, obsDist_IR2, obsTick_IR2);
		  if (obsDist_IR2 > 80 || obsDist_IR2 < 0) break;
		  if (HAL_GetTick() - last_curTask_tick >= 10) {
...
void RobotMoveUntilRIGHTIROvershootST() { //Move until IR stops detecting. ST (Second Turn) after robot reaches the back of second obstacle
	angleNow = 0; gyroZ = 0;
	PIDConfigReset(&pidTSlow);
	PIDConfigReset(&pidSlow);
	PIDConfigReset(&pidFast);
	...
	__SET_ENCODER_LAST_TICK(&htim2,lastDistTick_L);
	  do {
		  __ADC_Read_Dist(&hadc3, dataPoint2, IR_data_raw_acc2, obsDist_IR2, obsTick_IR2);
		  if (obsDist_IR2 > 80 || obsDist_IR2 < 0) {
			  __GET_ENCODER_TICK_DELTA(&htim2, lastDistTick_L, dist_dL);
			  curDistTick += dist_dL;
			  tempDist = ((double)curDistTick/15435)*200; //convert ticks moved into dist
			  horiDist = tempDist;
			  osDelay(10);
			  break;
		  }
...
void RobotMoveUntilLEFTIROvershootFT() {  //Move until IR stops detecting. FT (First Turn) after robot detects arrow on second obstacle
	obsDist_IR1 = 0;
	angleNow = 0; gyroZ = 0;
	  last_curTask_tick = HAL_GetTick();
	  do {
		  __ADC_Read_Dist(&hadc1, dataPoint1, IR_data_raw_acc1, obsDist_IR1, obsTick_IR1);
		  if (obsDist_IR1 > 80 || obsDist_IR1 < 0) break;
		  if (HAL_GetTick() - last_curTask_tick >= 10) {
			  StraightLineMove(SPEED_MODE_2);
			  last_curTask_tick = HAL_GetTick();
		  }
...
void RobotMoveUntilLEFTIROvershootST() { //Move until IR stops detecting. ST (Second Turn) after robot reaches the back of second obstacle
	angleNow = 0; gyroZ = 0;
	PIDConfigReset(&pidTSlow);
	PIDConfigReset(&pidSlow);
	PIDConfigReset(&pidFast);
	...
	__SET_ENCODER_LAST_TICK(&htim2,lastDistTick_L);
	  do {
		  __ADC_Read_Dist(&hadc1, dataPoint1, IR_data_raw_acc1, obsDist_IR1, obsTick_IR1);
		  if (obsDist_IR1 > 80 || obsDist_IR1 < 0) {
			  __GET_ENCODER_TICK_DELTA(&htim2, lastDistTick_L, dist_dL);
			  curDistTick += dist_dL;
			  tempDist = ((double)curDistTick/15435)*200; //convert ticks moved into dist
			  horiDist = (tempDist+24)/2;
			  osDelay(10);
			  break;
		  }
		  if (HAL_GetTick() - last_curTask_tick >= 10) {
...
void RobotMoveUntilRIGHTIRHit() { //Move until IR detects. unused
	obsDist_IR1 = 1000;
	angleNow = 0; gyroZ = 0;
	  last_curTask_tick = HAL_GetTick();
	  do {
		  __ADC_Read_Dist(&hadc3, dataPoint2, IR_data_raw_acc2, obsDist_IR2, obsTick_IR2);
		  if (obsDist_IR1 < 50) break;
		  if (HAL_GetTick() - last_curTask_tick >= 10) {
			  //StraightLineMove(SPEED_MODE_2);
			  last_curTask_tick = HAL_GetTick();
		  }
...
void RobotMoveUntilLEFTIRHit() { //Move until IR detects. unused
	obsDist_IR2 = 1000;
	angleNow = 0; gyroZ = 0;
	  last_curTask_tick = HAL_GetTick();
	  do {
		  __ADC_Read_Dist(&hadc1, dataPoint1, IR_data_raw_acc1, obsDist_IR1, obsTick_IR1);
		  if (obsDist_IR2 < 40) break;
		  if (HAL_GetTick() - last_curTask_tick >= 10) {
			  StraightLineMove(SPEED_MODE_2);
			  last_curTask_tick = HAL_GetTick();
		  }
...
void runOledTask(void *argument)
{
 ...
 //float angleTemp;
  ...
  {
	//batteryVal = 70;
	snprintf(ch, sizeof(ch), "Battery: %-3d%%", (int)batteryVal);
	OLED_ShowString(0, 0, (char *) ch);
//	angleTemp = angleNow / GRYO_SENSITIVITY_SCALE_FACTOR_2000DPS * 0.01;
	snprintf(ch, sizeof(ch), "angle:%4d", (int)retAngle);
	OLED_ShowString(0, 10, (char *) ch);
//	OLED_ShowString(0, 10, (char *) rxMsg);
//	OLED_ShowString(0, 20, (char *) aRxBuffer);
	snprintf(ch, sizeof(ch), "HoriDist: %4d", (int) horiDist);
	OLED_ShowString(0, 30, (char *) ch);
	snprintf(ch, sizeof(ch), "VertDist: %3d", (int) vertDist);
	OLED_ShowString(0, 40, (char *) ch);
	snprintf(ch, sizeof(ch), "RetDist: %4d", (int) retDist);
	OLED_ShowString(0, 50, (char *) ch);
//	snprintf(ch, sizeof(ch), "IR1:%-3d|IR2:%-3d", (int)obsDist_IR1, (int)obsDist_IR2);
//	OLED_ShowString(0, 50, (char *) ch);
	OLED_Refresh_Gram();
...
//	  	  	  break;
		...
		case 6: //BR manual
	  	 ...
	  	 case 12: // TR
			...
			__ACK_TASK_DONE(&huart3, rxMsg);
	  		 break;
	  	 case 13: // Run to test if IR is working
	  		 curTask = TASK_ADC;
	  		 break;
...
void runADCTask(void *argument)
{
  /* USER CODE BEGIN runADCTask */
	uint16_t dataPoint1 = 0; uint32_t IR_data_raw_acc1 = 0;
	uint16_t dataPoint2 = 0; uint32_t IR_data_raw_acc2 = 0;
  /* Infinite loop */
  for(;;)
  {
	  if (curTask != TASK_ADC) osDelay(1000);
	  else {
			dataPoint1 = 0; IR_data_raw_acc1 = 0; obsDist_IR1 = 1000;
			dataPoint2 = 0; IR_data_raw_acc2 = 0; obsDist_IR2 = 1000;
			last_curTask_tick = HAL_GetTick();
			__PEND_CURCMD(curCmd);
			do {
				///Remember to set obsDist_IR1 and obsDist_IR2 to display on OLED
				__ADC_Read_Dist(&hadc1, dataPoint1, IR_data_raw_acc1, obsDist_IR1, obsTick_IR1);
				__ADC_Read_Dist(&hadc3, dataPoint2, IR_data_raw_acc2, obsDist_IR2, obsTick_IR2);
			  osDelay(5); //(int)obsDist_IR1, (int)obsDist_IR2);
			} while (1);
		  HAL_ADC_Stop(&hadc1);
		  HAL_ADC_Stop(&hadc3);
		  clickOnce = 0;
...
void runFastestPathTask(void *argument)
{
  ...
  for(;;)
  {
	  if (curTask != TASK_FASTESTPATH) osDelay(1000);
		  ...
		  //if (step == 0) {
			  targetDist = 30;
			  RobotMoveDistObstacle(&targetDist, SPEED_MODE_2, 1);
		  //} else if (step == 1) {
...
//		  osDelay(3000); // video demo only
		  ...
		  case 30: // FL30 (4x2)
			  ...
			  osDelay(10);
			  break;
		  case 20: // FL20 For Video
			  __SET_CMD_CONFIG(cfgs[CONFIG_FL20], &htim8, &htim1, targetAngle);
			  osDelay(10);
			  break;
		  default: // FL00 (indoor 2x2 turn) Used for TASK 1
			  targetDist = 2;
			  RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_T);
			  osDelay(10);
...
//		  osDelay(3000); // video demo only
		  ...
		  case 30: // FR30 (4x2)
			  ...
			  osDelay(10);
			  break;
		  case 20: // FR20 For Video
			  __SET_CMD_CONFIG(cfgs[CONFIG_FR20], &htim8, &htim1, targetAngle);
			  osDelay(10);
			  break;
		  default: // FR00 (indoor 2x2) Used for TASK 1
			  targetDist = 3;
			  RobotMoveDist(&targetDist, DIR_BACKWARD, SPEED_MODE_T);
			  osDelay(10);
...
//		  osDelay(3000); // video demo only
		  ...
		  case 20: // BL20 (outdoor 3x1)
			  ...
			  osDelay(10);
			  break;
		  default: // BL00 (indoor 2x2) Used for TASK 1
			  targetDist = 10;
			  RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_T);
			  osDelay(10);
...
//		  osDelay(3000); // video demo only
		  ...
		  case 20: // BR20 (outdoor 3x1)
			  ...
			  osDelay(10);
			  break;
		  default: // BR00 (indoor 2x2) Used for TASK 1
			  targetDist = 13;
			  RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_T);
			  osDelay(10);
...
void runFastestPathTask_V2(void *argument)
{
  ...
  {//task2
	  if (curTask != TASK_FASTESTPATH_V2) osDelay(1000);
		  ...
		  int dir = curCmd.val;
		  int home = 0;
		  float first_obs_leeway = 3;
		  ////////////////First Obstacle INDOOR///////////////////
		  switch (dir) {
		  case 01: //01 turn right
			  targetDist = 30;	//From 35 to 25
			  //RobotMoveDistObstacle(&targetDist, speedModeFP, 0);
			  FASTESTPATH_TURN_RIGHT_90();
			  // Move forward abit for leeway
			  targetDist = first_obs_leeway;
			  RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_1);
...
//			  vertDist += 50; //to accommodate for the distance robot stopped at + width of first obstacle
//			  FASTPATH_RIGHT_OBS1();
//			  FASTPATH_STRAIGHTRIGHT_OBS1();
			  break;
		  case 02: //02 turn left
			  targetDist = 30;	//From 35 to 30
			  			  //RobotMoveDistObstacle(&targetDist, speedModeFP, 0);
...
//			  RobotMoveDistObstacle(&targetDist, speedModeFP, 0);
//			  FASTPATH_LEFT_OBS1();
//			  FASTPATH_STRAIGHTLEFT_OBS1();
			  break;
			  ////////////////////////////////////////////////////////
			  ////////////////First Obstacle OUTDOOR///////////////////
		  case 11: //01 turn right
			  targetDist = 30;	//From 35 to 25
			  //RobotMoveDistObstacle(&targetDist, speedModeFP, 0);
			  FASTESTPATH_TURN_RIGHT_90();
			  // Move forward abit for leeway
...
//			  FASTPATH_STRAIGHTLEFT_OBS1();
			  break;
		  ///////////////////////////////////////////////////////
		  /////////////Second Obstacle INDOOR////////////////////
		  case 21: //turn right
			  targetDist = 25;
			  //RobotMoveDistObstacle(&targetDist, speedModeFP, 1);
			  FASTESTPATH_TURN_RIGHT_90();
			  RobotMoveUntilLEFTIROvershootFT();
			  RobotMoveUntilLEFTIROvershootST();
			  // Move forward abit for leeway
			  targetDist = 5;
			  RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_1);
			  FASTESTPATH_TURN_LEFT_180();
		  ...
		  case 22://turn left
			  ...
			  //RobotMoveDistObstacle(&targetDist, speedModeFP, 1);
			  FASTESTPATH_TURN_LEFT_90();
			  RobotMoveUntilRIGHTIROvershootFT();
			  // Move forward abit for leeway
			  targetDist = 5;
			  RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_1);
			  FASTESTPATH_TURN_RIGHT_180();
			  RobotMoveUntilRIGHTIROvershootST();
			  // Move forward abit for leeway
			  targetDist = 5;
			  RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_1);
			  FASTESTPATH_TURN_RIGHT_90();
		  ...
		  //double fakeAngle = 0;
		  float temp_angle = 0;
		  float first_obs_ret_clearance = 14;
		  float ret_carpark_angle = 5;
		  switch (home){
		  	  case 1: //on right side of obstacle ******INDOOR*****
		  		 vertDist += 24 + 25;
		  		horiDist -= 20; // We want the robot to stop half box distance beside the 1st obstacle
		  		__CALCULATE_RETURN_DIST(vertDist, horiDist, retDist);
			  	__RETURN_HOME_ANGLE_RIGHT(vertDist, horiDist, retAngle);
			  	osDelay(20);

			  	temp_angle = retAngle;
			  	TURN_LEFT_X(temp_angle);

			  	if (retDist > 100) {
			  				  		targetDist = retDist - 24;

			  			  		} else {
			  				  		targetDist = retDist - 0.5*24;

			  			  		}
		  		RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_1); // move hypo

		  		// Turn left to center
		  		temp_angle = retAngle;
		  		TURN_RIGHT_X(temp_angle);

		  		// clear the first obstacle by moving robot length + box length

		  		targetDist = first_obs_ret_clearance;
		  		RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_1);

		  		//  turn left abit
		  		temp_angle = ret_carpark_angle;
		  		TURN_LEFT_X(temp_angle);
		  	    ...
		  	    osDelay(10);
		  	   break;
		  	  case 2: //on left side of obstacle ******OUTDOOR*****
		  		  vertDist += 24 + 25;
		  		  horiDist -= 20; // We want the robot to stop half box distance beside the 1st obstacle
		  		  __CALCULATE_RETURN_DIST(vertDist, horiDist, retDist);
			  	__RETURN_HOME_ANGLE_RIGHT(vertDist, horiDist, retAngle);
		  		...
		  		osDelay(10);
		      break;
		  	  case 3://on left side of obstacle ******INDOOR*****
		  		vertDist += 24 + 25;
		  		horiDist -= 20; // We want the robot to stop half box distance beside the 1st obstacle
		  		__CALCULATE_RETURN_DIST(vertDist, horiDist, retDist);
			  	__RETURN_HOME_ANGLE_RIGHT(vertDist, horiDist, retAngle);
		  	    ...
		  	    osDelay(10);
		  	   break;
		  	 case 4: //on left side of obstacle ******OUTDOOR*****
		  		 vertDist += 24 + 25;
		  		 horiDist -= 20; // We want the robot to stop half box distance beside the 1st obstacle
		  		 __CALCULATE_RETURN_DIST(vertDist, horiDist, retDist);
		  		 __RETURN_HOME_ANGLE_RIGHT(vertDist, horiDist, retAngle);
		  		 ...
		  		 targetDist = retDist - 0.6*24;	//From 0.8 to 0.6
		  		 RobotMoveDist(&targetDist, DIR_FORWARD, SPEED_MODE_1); // move hypo
